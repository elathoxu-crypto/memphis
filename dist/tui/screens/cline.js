...
export function renderCline(store) {
    const logs = store.readChain("journal");
    const clineLogs = logs
        .filter((b) => b.data?.content?.includes("[cline]"))
        .slice(-10)
        .reverse();
...
export function setupClineInput(store, widgets, onDone) {
    const { inputBox, inputField, contentBox, screen } = widgets;
    setTimeout(() => {
        inputBox.show();
        inputField.options.placeholder = "Wpisz prompt Cline:";
        inputField.focus();
        inputField.readInput((_err, value) => {
            if (validateInput(value)) {
                ...
                contentBox.setContent(`{white}Komenda:{/white} ${value.trim()}\n\n` +
