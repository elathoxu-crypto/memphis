...
function findDecision(blocks, id) {
    ...
    for (const block of blocks) {
        ...
        if (!parsed.ok)
            continue;
        if (parsed.value.recordId === needle)
            return { block, decision: parsed.value };
    }
    ...
    for (const block of blocks) {
        ...
        if (!parsed.ok)
            continue;
        if (parsed.value.decisionId === needle)
            return { block, decision: parsed.value };
    }
    ...
    for (const block of blocks) {
        ...
        if (!parsed.ok)
            continue;
        if (parsed.value.decisionId.startsWith(needle))
            return { block, decision: parsed.value };
    }
    return null;
}
export async function showCommand(kind, id) {
    ...
    const found = findDecision(blocks, id);
    if (!found) {
