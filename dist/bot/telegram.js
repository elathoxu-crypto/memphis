...
export class MemphisBot {
    ...
    async start() {
        ...
        await this.sendMessage(parseInt(process.env.TELEGRAM_CHAT_ID || "0"), "✅ Memphis Bot uruchomiony!\n\nDostępne komendy:\n/ask [pytanie] - Zapytaj Memphis\n/journal [tekst] - Dodaj wpis\n/recall [słowo] - Szukaj w pamięci\n/status - Status\n/help - Pomoc");
        this.loop();
    }
    async loop() {
        while (true) {
    ...
    async handleMessage(text, chatId) {
        ...
        try {
            switch (cmd.toLowerCase()) {
                case "/start":
                case "/help":
                    await this.sendMessage(chatId, "🤖 Memphis Bot\n\nKomendy:\n/ask [pytanie] - Zapytaj Memphis\n/journal [tekst] - Dodaj wpis\n/recall [słowo] - Szukaj w pamięci\n/status - Status");
                    break;
                case "/status":
