 ...
 */
export function getVaultSecret(keyName, password, chainsPath) {
 ...
 */
export function isVaultInitialized(chainsPath) {
    const config = loadConfig();
    const basePath = chainsPath || config.memory?.path || path.join(process.env.HOME, ".memphis/chains");
    const store = new Store(basePath);
 ...
 * 3. Config file
 */
export async function getProviderApiKey(providerName, options) {
...
function getEnvKeyForProvider(providerName) {
    const mapping = {
...
// 4. Return result
*/
//# sourceMappingURL=vault-providers.js.map