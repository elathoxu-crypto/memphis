 ...
 */
export function ensureGitRepo(basePath) {
    if (!isGitAvailable())
        return false;
 ...
 */
export function commitBlock(basePath, block, options = {}) {
    if (!isGitAvailable()) {
 ...
 */
export function getRecentCommits(basePath, n = 10) {
    if (!isGitAvailable() || !existsSync(join(basePath, ".git")))
        return [];
    try {
        const result = spawnSync("git", ["log", `--max-count=${n}`, "--oneline", "--no-decorate"], { cwd: basePath, timeout: 5000 });
        if (result.status !== 0)
            return [];
 ...
 */
export function hasUncommittedChanges(basePath) {
    if (!isGitAvailable() || !existsSync(join(basePath, ".git")))
        return false;
    try {
