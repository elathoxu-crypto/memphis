 ...
 */
function shouldUseSummaries(question) {
 ...
 */
function getRecentSummaries(store, max = 2) {
 ...
 */
function buildContextWithSummaries(summaries, directHits, maxChars = 6000) {
    const lines = [
        "MEMPHIS CONTEXT",
        "=".repeat(40),
        "",
 ...
 */
function buildContextString(hits, maxChars = 6000) {
    const lines = [
        "MEMPHIS CONTEXT",
        "=".repeat(40),
        "",
 ...
 */
async function selectProvider(preferred, useVault = false, vaultPassword) {
 ...
 */
export async function askWithContext(store, opts) {
    const { question, chain = "ask", provider, includeVault = false, topK = 8, maxContextChars = 6000, since, tags, noSave = false, json: returnJson = false, 
    // Summary options (5.8)
    preferSummaries = false, noSummaries = false, summariesMax = 2, explainContext = false, } = opts;
