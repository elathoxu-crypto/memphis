 ...
 */
export function recall(store, query) {
    ...
    for (const chain of chains) {
        ...
        for (const block of blocks) {
            ...
            if (sinceMs && blockMs < sinceMs)
                continue;
            if (untilMs && blockMs > untilMs)
                continue;
            // Type filter
            if (query.type && block.data.type !== query.type)
                continue;
            // Tag filter
            if (query.tag && !block.data.tags?.includes(query.tag))
                continue;
            // Text score
