...
import { verifyBlock, validateBlockAgainstSoul } from "./chain.js";
/**
 * Quarantine a block file - move to .quarantine directory
 */
function quarantineFile(chainDir, filename) {
 ...
 */
function listBlockFiles(chainDir) {
 ...
 */
function parseBlockFile(chainDir, filename) {
    try {
 ...
 */
function reviseChain(store, chain, dryRun) {
 ...
 */
export function revise(store, options) {
