...
export class MemphisTUI {
  ...
  // LLM Provider
  private llmProvider: Provider | null = null;
  ...
  private headerBox: ReturnType<typeof createHeaderBox>;
  private sidebarBox: ReturnType<typeof createSidebarBox>;
  private contentBox: ReturnType<typeof createContentBox>;
  private inputBox: ReturnType<typeof createInputBox>;
  private inputField: ReturnType<typeof createInputField>;
  private statusBar: ReturnType<typeof createStatusBar>;

  constructor() {
    // Initialize core dependencies with error handling
    const configResult = safeSync(
      () => loadConfig(),
   ...
   */
  private handleCharKey(char: string): void {
    switch (char) {
      case "c":
  ...
  private async askLLM(question: string): Promise<string> {
    ...
    const result = await safeAsync(
      () => this.llmProvider!.chat(messages, {
   ...
   */
  private handleOpenClawInput(): void {
    sleep(TIMING.INPUT_DELAY_MS).then(() => {
   ...
   */
  private async handleComputeNegotiation(): Promise<void> {
    clearInput(this.inputField);
   ...
   */
  private async handleOpenClawMessage(message: string): Promise<void> {
    const result = await safeAsync(
      () => this.openclawBridge.sendMessage("openclaw-001", message),
   ...
   * Render journal logs
   */
  private renderJournalLogs(): void {
   ...
   */
  private handleOfflineInput(): void {
    sleep(TIMING.INPUT_DELAY_MS).then(() => {
   ...
   * Handle model switch
   */
  private handleModelSwitch(): void {
    clearInput(this.inputField);
    ...
    readInput(this.inputField, (err, modelValue) => {
      ...
      const validModels: string[] = [DEFAULT_MODELS.PRIMARY, ...DEFAULT_MODELS.FALLBACK];
   ...
   * Handle input error
   */
  private handleInputError(error: string): void {
