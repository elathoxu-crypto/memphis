...
export class MemphisTUI {
  ...
  private renderOffline(): void {
    ...
    setTimeout(() => {
      ...
      this.inputBox.show();
      (this.inputField.options as any).placeholder = "Press number or Enter:";
      this.inputField.focus();
      this.inputField.readInput((err: any, value: any) => {
        if (value && value.trim()) {
          const input = value.trim().toLowerCase();
          ...
          if (input === 'm') {
            // Switch model
            this.inputField.setValue("");
            (this.inputField.options as any).placeholder = "Enter model name (llama3.2:1b, llama3.2:3b, gemma3:4b):";
            this.inputField.readInput((err2: any, modelValue: any) => {
