...
export function renderOffline(store: Store, state: TUIState): string {
  ...
  return (
    `{bold}{cyan}📡 Memphis – Panel Offline{/cyan}{/bold}\n\n` +
    `${offlineIndicator}\n\n` +
    `{bold}🤖 LLM:{/bold}\n${llmStatus}\n\n` +
...
export function setupOfflineInput(
  store: Store,
  widgets: TUIWidgets,
  state: TUIState,
  ...
  setTimeout(() => {
    inputBox.show();
    (inputField.options as any).placeholder = "Wpisz numer lub klawisz:";
    inputField.focus();
    ...
    inputField.readInput((_err: any, value: any) => {
      ...
      if (input === "m") {
        inputField.setValue("");
        (inputField.options as any).placeholder = "Nazwa modelu (np. llama3.2:1b):";
        screen.render();

        inputField.readInput((_e2: any, modelValue: any) => {
          if (validateInput(modelValue)) {
            state.selectedModel = modelValue.trim();
            process.env.OLLAMA_MODEL = modelValue.trim();
            contentBox.setContent(
              `{green}✅ Model zmieniony na: ${modelValue.trim()}{/green}\n\nNaciśnij dowolny klawisz...`
            );
          }
          inputBox.hide();
          screen.render();
      ...
      if (input === "o") {
        ...
        contentBox.setContent(
          `{cyan}Tryb offline: ${state.offlineMode ? "WŁĄCZONY 📡" : "WYŁĄCZONY 🌐"}{/cyan}\n\nNaciśnij dowolny klawisz...`
        );
        inputBox.hide();
        screen.render();
        onDone();
      ...
      const numMap: Record<string, number> = { "1": 5, "2": 2, "3": 4, "4": 6 };
      if (numMap[input]) {
