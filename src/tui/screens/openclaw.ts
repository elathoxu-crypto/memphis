...
export function renderOpenClaw(bridge: OpenClawBridge): string {
...
export function setupOpenClawInput(
  store: Store,
  ...
  setTimeout(() => {
    inputBox.show();
    (inputField.options as any).placeholder = "[Enter]=wiadomość | [n]=negocjuj | [r]=logi:";
    inputField.focus();
    ...
    inputField.readInput((_err: any, value: any) => {
      ...
      if (input === "n") {
        inputField.setValue("");
        (inputField.options as any).placeholder = "Udział CPU w % (np. 40):";
        screen.render();

        inputField.readInput((_e2: any, shareVal: any) => {
      ...
      if (validateInput(value)) {
        bridge.sendMessage("openclaw-001", value.trim()).then(response => {
          contentBox.setContent(
            `{bold}Wysłano: "${truncate(value.trim(), 40)}"{/bold}\n\n` +
