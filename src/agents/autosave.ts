...
export class AutosaveAgent {
  ...
  status(): { running: boolean; lastSave: Date | null; interval: number; self: SelfStats } {
    return {
...
let agent: AutosaveAgent | null = null;

export function startAutosave(options?: AutosaveOptions): AutosaveAgent {
  if (agent) {
    agent.stop();
  }
